...
//  $\cvrb|Asub|$ is an $8\times{}\cvrb|M|$ matrix stored in column-major order, $32$-byte aligned.
//  $\cvrb|Bsub|$ is an $\cvrb|M|\times 4$ matrix stored in row-major order, $32$-byte aligned.
...
/** BEGIN HIDDEN **/
// Usage note: This function can be used as the inner loop of a $\cvrb|M|\times{}\cvrb|M|$ matrix multiply.
...
#pragma unroll(4)  ///\lilabel{pragma}
    ...
    // Want to do the prefetching one per cache line, so that's once every 8 iterations.
    assert(M % 8 == 0);
    for (size_t i = 0; i < M; i += 8) {
      if (1) _mm_prefetch((void*)&Bsub[i * 4 + M], _MM_HINT_T2); // prefetch for the next time we call this function loop
#define DO_I(i) {                                                                                                                                                 \
      __m256d a0 = *((__m256d*)&(Asub[i * 8]));         /* $\cvrb|Asub|_{0,i}, \cvrb|Asub|_{1,i}, \cvrb|Asub|_{2,i}, \cvrb|Asub|_{3,i}$                         */\
      __m256d b0 = *((__m256d*)&(Bsub[i * 4]));         /* $\cvrb|Bsub|_{i,0}, \cvrb|Bsub|_{i,1}, \cvrb|Bsub|_{i,2}, \cvrb|Bsub|_{i,3}$                         */\
      c0 = muladd(a0,  b0,  c0);                                                                                                                                  \
